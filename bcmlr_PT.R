...
bcmlr_PT <- function(data, num_iter = 15000, num_warmup = 5000, num_tune = 5000, num_temper = 40, init_CP, num_CP,  pc_cores = detectCores(), print_outputs = TRUE){
  ...
  ####### Multiple change point case #######
    ...
    DEO <- function(data, num_iter, schedule, init_CP, num_CP,  pc_cores = pc_cores){
    ...
    for (rd in 1:maxRound){
      ...
      rej = DEO(data = X, num_iter = n, init_CP = init_CP, num_CP = num_CP, schedule = T.set)
    ...
    # Number of copies of PT
    k.star = pc_cores/(num_temper+1)
    
    
    start = Sys.time()
    PT_copies = 1
    if (k.star > 1){
      PT_copies = round(k.star)
    }
    
    cat("Fitting", PT_copies, " copies of PT.....")
    
    for (k in 1:PT_copies){
      for (iter in 1:num_iter){
    ...
    if (print_outputs){
      cat("\n" ,"The algorithm took ", format(runTime), "to run ", k.star, " copies of PT with ", num_iter, 
