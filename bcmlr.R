# Held-out strategy:
# - Hold out every samples at indices multiple of an integer i (thinning = i) when fitting bclr.
# - Use the held-out data to compute AUC
# min_size is referred from lines 187, 188 in https://github.com/manilasoldier/bclr/blob/master/bclr/bclr_multi.py
# E.divisive uses min-size = 30, a heuristic number for sufficient sample size.
# alpha_f: for frequentist confidence intervals
bcmlr <- function(data, num_CP, init = "even", prior = "Gaussian", alpha_f = 0.1, 
  ...
  if (thinning != 1){
    ...
    X_auc = X_all[holdout_idx, ] # rows of X selected to calculate AUC
    X = X_all[-holdout_idx, ] # rows of X selected to fit the model  
  ...
  GS_update <- function(temper, beta, kappa, omega, tau_sq, xi, nu, lambda, X_all, X, train_idx, holdout_idx, n, N, p, L, J, model_selection){
    ...
    if (prior == "horseshoe"){
      ...
      for(j in 1:(J-1)){
        Sigma0_j = diag(lambda[,j]^2*tau_sq) 
