...
bcmlr <- function(data, num_iter = 10000, num_warmup = 5000, init_CP = NA, num_CP, thinning, print_outputs = FALSE, print_progress = FALSE){
  ...
  calculate_auc <- function(Y, P) {
    ...
    for (i in 1:nrow(df)) {
      ...
      #print(c(TP,FP, total_positives, total_negatives))
      #if (total_positives == 0){total_positives = 1}
      #if (total_negatives == 0){total_negatives = 1}
  ...
  GS_update <- function(temper, beta, kappa, omega, data, X, X_auc, N, L, J, m0, V0inv){
    ...
    for (j in 1:(J-1)){
      if(j == 1){
        ...
        auc_values[j] = calculate_auc(Y_auc, ratios)
      }else if (j == J-1){
