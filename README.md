...
The bcmlr function gives the options to 1) detect a fixed number of changepoints when the number of changes is known or 2) detect the number of changes when the number of changes is unknown. When the number of changes is unknown, one needs to use the bcmlr_model_select function that fits bcmlr to first detect the number of changes and generates posterior samples for posterior inference. 
