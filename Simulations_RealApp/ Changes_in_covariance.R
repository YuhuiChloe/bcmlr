...
sim_mvn_cic <- function(N, kappa0){
  p = 4
  v = 1
  s = 0.8
  Sig1 = matrix(data = c(v, s, 0, 0,
                         s, v, 0, 0,
                         0, 0, v, 0,
                         0, 0, 0, v), nrow = p, ncol = p, byrow = TRUE)
  Sig2 = matrix(data = c(v, 0, s, 0,
                         0, v, 0, 0,
                         s, 0, v, 0,
                         0, 0, 0, v), nrow = p, ncol = p, byrow = TRUE)
  X1 = mvrnorm(n = kappa0[1], mu = rep(0,p), Sigma = Sig1)
  X2 = mvrnorm(n = kappa0[2]- kappa0[1], mu = rep(0, p), Sigma = Sig2)
...
kappa0 = c(100, 500) 
N = 600 
data = sim_mvn_cic(N = N, kappa0 = kappa0)
dim(data$psi)
psi_parallel <- function(i){
  sim_mvn_cic(N = N, kappa0 = kappa0)
}
